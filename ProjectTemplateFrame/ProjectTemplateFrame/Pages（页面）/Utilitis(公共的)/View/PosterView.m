...
@interface PosterView ()<UIScrollViewDelegate>
{
    //页码
...
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        _timeLoop = 3;
        _isLoop = YES;
        _pageType = PosterViewPageTypePageCtl;
    }
    return self;
}
...
-(void)setPicIndication {
    ...
    if (_pageType==PosterViewPageTypePageCtl) {
        //    设置总页数
        _pageCtl.hidden = NO;
        self.pageCtl.numberOfPages = _itemArr.count;
    }else if (_pageType==PosterViewPageTypeLabel){
    ...
    }else if (_pageType==PosterViewPageTypeDetail) {
...
-(void)setDesItems:(NSArray<NSString *> *)desItems {
    _desItems = desItems;
    _pageType = PosterViewPageTypeDetail;
    [self setPicIndication];
